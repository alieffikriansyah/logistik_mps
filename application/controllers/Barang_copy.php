  ...
  public function index()
{
    ...
    $this->db->select('
        b.id_barang, b.nama_barang, b.foto_barang, b.merk, b.lokasi,
        COALESCE(SUM(bm.jumlah_masuk), 0) as total_masuk,
        COALESCE(SUM(bk.jumlah_keluar), 0) as total_keluar,
        (COALESCE(SUM(bm.jumlah_masuk), 0) - COALESCE(SUM(bk.jumlah_keluar), 0)) as stok_aktual,
        s.nama_satuan, j.nama_jenis
    ');
    $this->db->from('barang b');
    $this->db->join('satuan s', 's.id_satuan = b.satuan_id', 'left');
    $this->db->join('jenis j', 'j.id_jenis = b.jenis_id', 'left');
    $this->db->join('barang_masuk bm', 'b.id_barang = bm.barang_id', 'left');
    $this->db->join('barang_keluar bk', 'b.id_barang = bk.barang_id', 'left');
    
    // ❌ EXCLUDE STATUS 8 (HAPUS) - TAMBAH INI
    $this->db->where('bm.status !=', 8);
    $this->db->where('bk.kode !=', 8); // atau bk.status jika ada kolom status
    ...
    $this->db->group_by('b.id_barang, b.nama_barang, b.foto_barang, b.merk, b.lokasi, s.nama_satuan, j.nama_jenis');
